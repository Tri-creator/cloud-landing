...
export async function Footer() {
  ...
  const brandName = footerData?.brandName || 'VCV Cloud'
  const tagline = footerData?.tagline || t('footer.tagline', locale)
  const columns = footerData?.columns || []
  const socialLinks = footerData?.socialLinks || []
  const copyrightRaw = footerData?.copyrightText || t('footer.copyright', locale, { year })
  const copyright = copyrightRaw.replace('{year}', year)

  return (
    <footer className="vcv-footer mt-auto">
      <div className="vcv-footer-inner container">
        {/* Top row */}
        <div className="vcv-footer-top">
          {/* Brand */}
          <div className="vcv-footer-brand">
            <Link href="/" className="vcv-footer-logo">
              <svg viewBox="0 0 32 32" fill="none" className="w-8 h-8 shrink-0">
                ...
                />
                <circle cx="22" cy="11" r="3" fill="#fff" fillOpacity=".9" />
                <defs>
        ...
        {/* Bottom row */}
        <div className="vcv-footer-bottom">
          <p className="vcv-footer-copy">{copyright}</p>
          {socialLinks.length > 0 && (
            <div className="vcv-footer-socials">
              {socialLinks.map(({ platform, url }, i) => (
                <a
                  key={i}
                  href={url}
                  className="vcv-social-link"
                  aria-label={platform}
                  target="_blank"
